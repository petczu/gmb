@php
    ...
    $link = fn (string $page, string $label): string => '<a href="'.\App\Support\MarketingSite::legal($page).'" target="_blank" rel="noopener" style="'.$linkStyle.'">'.e($label).'</a>';
@endphp
...
<div style="max-width:26rem; margin:1.25rem auto 0; text-align:center;">
    <p style="font-size:.75rem; line-height:1.5; color:#9ca3af;">
        {!! __('auth.legal', [
            'terms' => $link('terms', __('auth.terms')),
            'privacy' => $link('privacy', __('auth.privacy')),
            'cookie' => $link('cookies', __('auth.cookie')),
        ]) !!}
    </p>
</div>
